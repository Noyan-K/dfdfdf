...
export class AuthService {
    ...
    async validateUser(email: string, pass: string): Promise<any> {
        ...
        const passwordMatches = user ? await bcrypt.compare(pass, user.password) : false;

        if (user && passwordMatches) {
            const userRoles = await this.rolesService.findAllUserRole(user.id, 0, undefined);
    ...
    async login(user: Omit<User, 'password'> | undefined): Promise<TokensInterface> {
        if (!user) {
            throw new ForbiddenException('User not found');
        }
        const receivedUsersRoles = await this.rolesService.findAllUserRole(user.id, 0, undefined);
        const payload = {
            email: user.email,
            sub: user.id,
            name: user.name,
    ...
    async refresh(userId: number): Promise<TokensInterface> {
        ...
        const receivedUsersRoles = await this.rolesService.findAllUserRole(userId, 0, undefined);
