...
export class DocumentService {
    ...
    async update(id: number, updateDocumentInput: UpdateDocumentInput): Promise<Document | null> {
    ...
    async uploadFile(file: Express.Multer.File): Promise<Document> {
        ...
        fs.writeFileSync(`${validationResult.destination}/${fileName}`, file.buffer);
    ...
    async uploadFiles(files: Express.Multer.File[]): Promise<Document[]> {
        ...
        const dataArr: Data[] = await files.map((file: Express.Multer.File): Data => {
            const fileName = `${uuidv4()}${extname(file.originalname)}`;
            fs.writeFileSync(`${validationResult.destination}/${fileName}`, file.buffer);
